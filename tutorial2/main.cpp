 ...
 <root main_tree_to_execute = "MainTree" >
     <BehaviorTree ID="MainTree">
        <Sequence name="root">
            ...
            <ThinkWhatToSay   text="{value}"/>
            <SaySomething     message="{value}" />
            <SaySomething2    message="SaySomething2 works too..." />
            <SaySomething2    message="{value}" />
        </Sequence>
     </BehaviorTree>
 </root>
 )";
...
int main() {
  ...
  // The class SaySomething has a method called providedPorts() that define the
  // INPUTS. In this case, it requires an input called "message"
  ...
  // therefore we have to pass the PortsList explicitly if we want the Action to
  // use getInput() or setOutput();
  ...
  * Blackboard entry.
  *
  * This means that ThinkSomething will write into the entry with key
  "value";
