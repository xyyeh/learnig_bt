...
/**
 * Sequence - restarts sequence when child returns failure, tick same child
 * again when child returns running (Usually for a 'sequence' of action)
 *
 * ReactiveSequence - restarts sequence when child returns failure or running
 * (Usually for continously check cond. and act)
 *
 * SequenceStar - tick same child again when child returns failure or running
 * (Usually for sequence where one node logically leads to the next)
 */
 ...
 <root main_tree_to_execute = "MainTree" >
     <BehaviorTree ID="MainTree">
        <Sequence name="root_sequence">
            ...
            <SaySomething   name="say_something"/>
        </Sequence>
     </BehaviorTree>
 </root>
 )";
...
int main() {
  ...
  // ExampleNodes::RegisterNodes(factory)
